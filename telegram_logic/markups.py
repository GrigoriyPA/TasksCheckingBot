...
def get_results_table(results, homework_name, homework_size, first_task_id):
    ...
    keyboard = []
    row = [types.InlineKeyboardButton(text=" ", callback_data="0"), types.InlineKeyboardButton(text="Σ", callback_data="0")]
    ...
    for result in results:
        row = [types.InlineKeyboardButton(text=result[0].login, callback_data="0")]
        task_id = 0
        current_sum = 0
        ...
        row = row[:1] + [types.InlineKeyboardButton(text=str(current_sum), callback_data="0")] + row[1:]
